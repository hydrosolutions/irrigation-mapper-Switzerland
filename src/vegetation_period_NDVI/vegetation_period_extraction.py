...
from src.vegetation_period_NDVI.time_series import get_harmonic_ts, get_harmonized_data
...
def apply_temporal_operation(
    ...
    def map_func_reverse(x):
        interval = intervals.get(ee.Number(x))
        start_date = ee.Date(ee.List(interval).get(0))
        end_date = ee.Date(ee.List(interval).get(1))
        filtered_collection = collection.filterDate(
            start_date.advance(-AGG_INTERVAL, "day"), end_date
        )
        return operation(filtered_collection, x)

    sequence_mapped = (
        ee.List.sequence(0, intervals.size().subtract(1)).map(map_func)
        if direction == 1
        else ee.List.sequence(intervals.size().subtract(1), 0, -1).map(map_func_reverse)
    )

    return ee.ImageCollection(sequence_mapped).reduce(ee.Reducer.firstNonNull())
...
def find_start_first_veg_period(
    ...
    def operation(filtered_collection: ee.ImageCollection, x: ee.Number) -> ee.Image:
        ...
        return (
            ee.Image(ee.Number(x).subtract(1).max(0)).int().updateMask(sum_image.eq(2))
        )
...
def find_start_second_veg_period(
    ...
    def operation(filtered_collection: ee.ImageCollection, x: ee.Number) -> ee.Image:
        sum_image = filtered_collection.sum()
        return (
            ee.Image(ee.Number(x).subtract(1).max(0))
            .int()
            .updateMask(sum_image.eq(2))
...
def combine_results_v2(
    ...
    result = (
        ...
        .addBands(
            second_start.rename("secondStart")
            .addBands(second_end.rename("secondEnd"))
            # .updateMask(double_cropping.mask()) # Commented out to keep information on sowing date of winter crops
        )
    )
...
def get_crop_veg_period(
    year: int, aoi: ee.Geometry, time_intervals: ee.List,NDVI_THRESHOLD: float, NDVI_LOW_THRESHOLD: float
) -> ee.Image:
    """
    ...
    Args:
        ...
        NDVI_LOW_THRESHOLD (float): Low NDVI threshold for vegetation.
